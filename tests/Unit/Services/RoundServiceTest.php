...
class RoundServiceTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->cardService->shouldReceive('drawCards')
                ->with(13)
                ->andReturn($mockCards);
        $this->game = Game::factory()->withGamePlayers()->create();
    }
    ...
    public function testCalculateScoresMoonShot(): void
    {
        ...
        for ($i = 0; $i < 12; $i++) {
            ...
            $trick->shouldReceive('winningGamePlayer')->andReturn($player3);
            $trick->shouldReceive('getTrickPoints')->andReturn(1);
            $trick->shouldReceive('discards')->andReturn(collect([1, 2, 3, 4]));
            $tricks->push($trick);
        }
        $trick = Mockery::mock(Trick::class);
        $trick->shouldReceive('winningGamePlayer')->andReturn($player3);
        ...
        $tricksMock = Mockery::mock(Trick::class);
        $tricksMock->shouldReceive('count')->andReturn($tricks->count());
        $tricksMock->shouldReceive('latest')->andReturnSelf();
        $tricksMock->shouldReceive('first')->andReturn($tricks->first());

        $round->shouldReceive('getAttribute')
            ->with('tricks')
            ->andReturn($tricks);
        $round->shouldReceive('tricks')->andReturn($tricksMock);
...
//        $this->assertEquals(26, $scores[2]);
    ...
    public function testCalculateScores(): void
    {
        ...
        for ($i = 0; $i < 5; $i++) {
            $trick = Mockery::mock(Trick::class);
            $trick->shouldReceive('winningGamePlayer')->andReturn($players->random());
        ...
        $trick = Mockery::mock(Trick::class);
        $trick->shouldReceive('winningGamePlayer')->andReturn($player4);
        $trick->shouldReceive('getTrickPoints')->andReturn(14);
        $tricks->push($trick);
        ...
        $trick = Mockery::mock(Trick::class);
        $trick->shouldReceive('winningGamePlayer')->andReturn($player1);
        $trick->shouldReceive('getTrickPoints')->andReturn(1);
        $tricks->push($trick);

        $round->shouldReceive('getAttribute')->with('tricks')->andReturn($tricks)->getMock();
        $trickMock = Mockery::mock(Trick::class);
        $trickMock->shouldReceive('count')->andReturn($tricks->count());
        $trickMock->shouldReceive('latest')->andReturnSelf();
        $trickMock->shouldReceive('first')->andReturnSelf();
        $trickMock->shouldReceive('discards')->andReturn(collect([1, 2, 3, 4]));
        $round->shouldReceive('tricks')->andReturn($trickMock);
