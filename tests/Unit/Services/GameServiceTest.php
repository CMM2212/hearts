...
class GameServiceTest extends TestCase
{
    ...
    public function testCreateGame(): void
    {
        ...
        $exceptedGamePlayerId = 4;
        $playerId = 4;
        $roundService = Mockery::mock(RoundService::class);
        $gameService = new GameService($roundService);
        $creationResult = $gameService->createGame($playerId);
        $game = $creationResult['game'];
        $humanGamePlayer = $creationResult['humanGamePlayer'];

        $this->assertEquals($expectedGameId, $game->id);
        $this->assertEquals($exceptedGamePlayerId, $humanGamePlayer->id);
    ...
    public function testCreateGamePlayersSeatsUnique(): void
    {
        ...
        $roundService = Mockery::mock(RoundService::class);
        $gameService = new GameService($roundService);
        $game = $gameService->createGame($playerId)['game'];
    ...
    // Humans are assigned to the last seat (seat 4)
    public function testCreateGamePlayersIsHuman(): void
    {
        ...
        $game->gamePlayers()->get()->each(function ($gamePlayer) {
            if ($gamePlayer->seat_number === 4)
                $this->assertTrue((bool) $gamePlayer->is_human);
            else
