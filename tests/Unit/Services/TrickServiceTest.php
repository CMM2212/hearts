...
class TrickServiceTest extends TestCase
{
    ...
    public function testGetNextPlayerThirdDiscard()
    {
        ...
        $trick = Trick::factory()->create(['round_id' => $round->id]);

        $hand1 = $round->hands->get(2);
        $card1 = Card::factory()->create(['suit' => 'clubs', 'rank' => 'five']);
        $cardHand1 = CardHand::factory()->create(['hand_id' => $hand1->id, 'card_id' => $card1->id]);
        $discard1 = Discard::factory()->create(['trick_id' => $trick->id, 'cardhand_id' => $cardHand1->id]);

        $hand2 = $round->hands->get(3);
        $card2 = Card::factory()->create(['suit' => 'clubs', 'rank' => 'six']);
        $cardHand2 = CardHand::factory()->create(['hand_id' => $hand2->id, 'card_id' => $card2->id]);
        $discard2 = Discard::factory()->create(['trick_id' => $trick->id, 'cardhand_id' => $cardHand2->id]);
